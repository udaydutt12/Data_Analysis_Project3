...
# Multiple Linear Regression to Predict 'phd' for absence or presence of heart disease.
...
# The summary of linear regression explains that there are few significant predictive 
# variables that are useful. However, R squared value explains only 50% of variations 
# in this model.  Also the summary of predicted values indicates that there are values 
# less than zero and greater than one, which reveals that it is not a good model since 
# phd variables only contain values from zero to one. 

# For residual analysis, we plotted fitted values versus residuals of the model. As we 
# expected, the residual plot explains that this is a poor method of predicting responsive 
# variables. A residual plot of a good model indicates a random and dispersed patterns of 
# dots; however, our graph shows some patterns, which explain that some of the variables 
# have poor t values. Thus, other models such as classification is more appropriate for this data. 
